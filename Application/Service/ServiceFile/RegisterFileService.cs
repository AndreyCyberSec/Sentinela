...
namespace Application.Service.ServiceFile
{
    public class RegisterFileService : IFileRegister
    {
        ...
        public async Task CheckPortResult(IReadOnlyList<NetScannerEntity> scanResults, string? outPutDirectory = null)
        {
            if (scanResults == null || scanResults.Count == 0)
                throw new ArgumentException("Scan results must be provided.", nameof(scanResults));

            Directory.CreateDirectory(outPutDirectory ?? AppContext.BaseDirectory);
            foreach(var item in scanResults)
            {
                var fileName = $"PortCheck_{item.HostName}_{item.Port}.txt";
                var fullPath = Path.Combine(outPutDirectory ?? AppContext.BaseDirectory, fileName);

                var targetHost = await netScannerService.PortCheck(item.HostName, item.Port, timeOutMs: 2000);
